...
<body>
  <main class="page">
    <header class="header">
      <div><h1>ID100 Battery Monitor</h1><div class="sub" id="systemInfo">Loading device status...</div></div>
    ...
    <section class="grid">
      <article class="card wide">
        ...
        <div class="form-grid">
          <div><label for="gatewayName">Gateway name</label><input id="gatewayName" maxlength="60" required></div>
          <div><label for="tester">Tester name</label><input id="tester" maxlength="60" required></div>
          <div><label for="location">Test location</label><input id="location" maxlength="100" required></div>
        </div>
        <div class="actions"><button id="saveTest">Save information</button><span class="message" id="testMessage"></span></div>
      </article>
  ...
  <script>
    ...
    function batteryCard(battery,sampling){
      const percent=Number(sampling?.displayed_battery_percent??battery.battery_percent).toFixed(1);
      const state=sampling?.state||'idle';
      return `<div class="battery-head"><div><h2>${escapeHtml(battery.device_id)}</h2><div class="muted">GPIO${battery.gpio} · ${escapeHtml(battery.device_name)}</div><span class="state ${state}">${state}</span></div><div class="percent">${percent}%</div></div><div class="bar"><span style="width:${percent}%"></span></div><div class="metrics"><div class="metric">Battery voltage<b>${(battery.battery_voltage_mv/1000).toFixed(3)} V</b></div><div class="metric">ADC voltage<b>${battery.adc_voltage_mv} mV</b></div><div class="metric">ADC raw<b>${battery.adc_raw}</b></div><div class="metric">Battery status<b>${escapeHtml(battery.status||'—')}</b></div></div>`;
    }

    async function refreshStatus(){
      try{
        const status=await fetch('/api/status').then(response=>response.json());
        status.batteries.forEach((battery,index)=>byId(`battery${index}`).innerHTML=batteryCard(battery,status.sampling_channels?.[index]));
        ...
        const states=(status.sampling_channels||[]).map(channel=>`CH${channel.channel}: ${channel.state}`).join(' · ');
        byId('systemInfo').textContent=`Firmware ${status.firmware_version} · ${status.time} · ${states}`;
        ...
        if(wifiAttempt){
          const now=Date.now();
          if(now-wifiAttempt.startedAt>1500&&wifiOnline&&status.sta_ssid===wifiAttempt.ssid){
            wifiAttempt=null;
      ...
      }catch(error){byId('onlineState').textContent='OFFLINE';}
    }
    ...
    byId('connectWifi').onclick=async()=>{
      ...
      if(!ssid){return;}
      const body=new URLSearchParams({ssid,password:byId('password').value});
      wifiAttempt={ssid,startedAt:Date.now(),deadline:Date.now()+10000};
      showWifiMessage('');
      try{
        const response=await fetch('/api/wifi',{method:'POST',body});
        if(!response.ok){wifiAttempt=null;}
      }catch(error){wifiAttempt=null;}
    };
